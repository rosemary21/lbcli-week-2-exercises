...
UTXO_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq '(.vout[0].value * 100000000) | round')
...
CHANGE_AMOUNT=$((UTXO_VALUE - PAYMENT_AMOUNT - FEE_SATS))  # = 1428798
check_cmd "Change calculation" "CHANGE_AMOUNT" "$CHANGE_AMOUNT"

# Convert amounts to BTC for createrawtransaction
PAYMENT_BTC=$(printf "%.8f" $(echo "scale=8; $PAYMENT_AMOUNT/100000000" | bc))  # 0.15000000
CHANGE_BTC=$(printf "%.8f" $(echo "scale=8; $CHANGE_AMOUNT/100000000" | bc))    # 0.01428798
...
VERIFY_PAYMENT=$(echo "$DECODED_TX" | jq -r --arg addr "$PAYMENT_ADDRESS" \
  '.vout[] | select(.scriptPubKey.address == $addr) | .value' | xargs printf "%.8f")
...
CHILD_SEND_AMOUNT=$((CHANGE_AMOUNT - CHILD_FEE_SATS))  # = 1428798 - 2180 = 1426618
...
SECONDARY_OUTPUT_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$SECONDARY_TX" | jq '(.vout[1].value * 100000000) | round')
...
TIMELOCK_AMOUNT=$((SECONDARY_OUTPUT_VALUE - TIMELOCK_FEE))  # = 39648
